...
class EntitySerializer[T: Manifest] extends CustomSerializer[Entity[Int, Persisted, Class[T]]](formats => ( {
  ...
  case u: User =>
    ...
    Extraction.decompose(u.copy()) removeField {
      case ("User", _) => true
      case _ => false
    } merge render("events" -> Extraction.decompose(u.getAssociatedEvents(new EventUserRepository)) //TODO Get rid of nasty coupling to repository implementation. How to mix in Injectable?
    )
  case t: Guild =>
