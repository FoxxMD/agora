...
object TournamentDetailsEntity extends Entity[Int, NoId, TournamentDetails]("tournamentdetails") {
  ...
  val timeStart = column("timeStart") to { details => (details.timeStart.getMillis/1000).toInt }
  val timeEnd = column("timeEnd") to  { details => (details.timeEnd.getMillis/1000).toInt }

  def constructor(implicit m: ValuesMap) = {
    val ts = new DateTime(m(timeStart)*1000L,DateTimeZone.UTC)
    val te = new DateTime(m(timeEnd)*1000L,DateTimeZone.UTC)
    new TournamentDetails(tournament, name, gamePlayed, description, rules, prizes, streams, servers, ts, te) with Stored
  }
}
