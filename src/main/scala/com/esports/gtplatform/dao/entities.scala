...
object EventDetailsEntity extends Entity[Int, SurrogateIntId, EventDetails]("eventdetails") {
  ...
  def constructor(implicit m: ValuesMap) = new EventDetails(address, city, state, description, rules, prizes, streams, servers, timeStart, timeEnd) with Stored
  {
    val id:Int = EventDetailsEntity.id
  }
}
