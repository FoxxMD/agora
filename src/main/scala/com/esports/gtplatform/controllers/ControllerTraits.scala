...
trait RESTController extends BasicServletWithLogging with JacksonJsonSupport with MethodOverride with Injectable {
  ...
  * Injectable = Support for injecting dependencies using subcut */
  ...
  protected implicit val jsonFormats: Formats = DefaultFormats ++ GTSerializers.mapperSerializers ++ org.json4s.ext.JodaTimeSerializers.all + new org.json4s.ext.EnumNameSerializer(GameType) + new org.json4s.ext.EnumNameSerializer(JoinType) + new com.esports.gtplatform.json.DateSerializer
  before() {
