...
class TeamController(implicit val bindingModule: BindingModule) extends APIController {
  ...
  post("/:id/removeMember") {
    val teamId = params.getOrElse("id", halt(401, "Team Id parameter is missing"))
    val removingUser = parsedBody.\("userId").extractOrElse[Int](halt(401, "User Id parameter is missing"))
    auth()
    teamRepo.get(teamId.toInt) match {
      case Some(t: Team with Persisted) =>
        if (user.role == "admin" || t.getCaptain == user) {
            ...
            userRepo.get(removingUser) match {
              case Some(u: User) =>
                if(t.teamPlayers.exists(x => x.user == u)) {
                  teamRepo.update(t, t.removeUser(u))
                }
                else{
